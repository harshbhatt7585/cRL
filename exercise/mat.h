...
static inline void clear(matrix* mat) {
    memset(mat->data, 0, sizeof(f32) * (u64)mat->rows * mat->cols);
}

static inline void fill(matrix* mat, f32 x) {
...
static inline void scale(matrix* mat, f32 x) {
    u64 size = (u64)mat->rows * mat->cols;
    for (u64 i = 0; i < size; i++) {
        mat->data[i] *= x;
    }
...
static inline b32 add(matrix* out, const matrix* a, const matrix* b) {
...
static inline b32 sub(matrix* out, const matrix* a, const matrix* b) {
    if (a->rows != b->rows || a->cols != b->cols) { return false; }
    if (out->rows != a->rows || out->cols != a->cols) { return false; }
...
static inline b32 relu(matrix* out, const matrix* in) {
...
static inline b32 softmax(matrix* out, const matrix* in) {
    if (out->rows != in->rows || out->cols != in->cols) { return false; }
...
static inline b32 cross_entropy(matrix* out, const matrix* p, const matrix* q) {
...
static inline b32 relu_add_grad(matrix* out, const matrix* in, const matrix* grad) {
    if (out->rows != in->rows || out->cols != in->cols) { return false; }
...
static inline b32 softmax_add_grad(
...
static inline b32 cross_entropy_add_grad(
